...
namespace Universe.Data.Client.Player {
	...
	*/
	public class PlayerState : MonoBehaviour {
		...
		PlayerInventory Inventory => (PlayerInventory) _gameState.InventoryController.GetInventory(Data.InventoryUid);
		...
		public class PlayerInputController {
			...
			void HandleBlockSelectionInput() {
				...
				if(scroll > 0f) {
					_playerState.Inventory.SelectNext();
				} else if(scroll < 0f) {
					_playerState.Inventory.SelectPrevious();
				}
			}
			...
			public void PlaceBlock() {
				InventorySlot slot = _playerState.Inventory.GetSelectedSelectedSlot();
				...
				if(hit.valid) {
					...
					_playerState.Inventory.slots[_playerState.Inventory.GetSelectedSelectedSlotIndex()] = slot;
			...
			public void RemoveBlock() {
				...
				if(hit.valid) {
					...
					_playerState.Inventory.AddToAnySlot(removedType, 1);
					// Mesh rebuild will be requested by the modification queue after applying batched changes
				}
			}
		}
