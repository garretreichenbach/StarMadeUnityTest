...
namespace Universe.Data.Chunk {
    ...
    public class GPUCompressionManager {
        ...
        private ChunkMemoryManager MemoryManager => ChunkMemoryManager.Instance;
        private ComputeShader CompressionShader => MemoryManager.compressionShader;
        private ComputeBuffer GpuInputBuffer => MemoryManager._gpuInputBuffer;
        private ComputeBuffer GpuOutputBuffer => MemoryManager._gpuOutputBuffer;
        private ComputeBuffer GpuMetadataBuffer => MemoryManager._gpuMetadataBuffer;
        ...
        public async Task<ChunkMemoryManager.CompressedChunk> CompressChunkAsync(long chunkID) {
            ...
            var metaRequest = AsyncGPUReadback.Request(GpuMetadataBuffer);
            ...
            var dataRequest = AsyncGPUReadback.Request(GpuOutputBuffer, 0, elementCount);
            ...
            NativeArray<byte> pool = MemoryManager._compressedPool;
            for (int i = 0; i < compressedSize; i++)
                pool[offset + i] = compressedData[i];
            MemoryManager._compressedPoolHead += compressedSize;

            // Update allocation and header
            if (!MemoryManager._allocations.TryGetValue(chunkID, out var alloc))
            ...
            if (MemoryManager._headers.TryGetValue(chunkID, out var header)) {
        ...
        public async Task<bool> DecompressChunkAsync(long chunkID) {
            ...
            // Upload to GPU
            using (var gpuCompressedBuffer = new ComputeBuffer((compressedSize + 3) / 4, sizeof(int))) {
                ...
                using (var gpuOutputBuffer = new ComputeBuffer(32 * 32 * 32, sizeof(int))) {
                    // Prepare metadata buffer
                    using (var gpuMetadataBuffer = new ComputeBuffer(2, sizeof(uint))) {
                        ...
                        var decompressionShader = MemoryManager.decompressionShader;
                        ...
                        var readback = AsyncGPUReadback.Request(gpuOutputBuffer);
            ...
            // Free compressed slot (optional: add to free list for reuse)
            // (For now, just mark as available by not using the offset/size)
