...
public class ObjectPoolAdvanced<T> {
	...
	public ObjectPoolAdvanced(Class<T> cls, int limit) {
		this.cls = cls;
		this.limit = limit;
	}
	...
	public static <T> ObjectPoolAdvanced<T> get(Class<T> cls, int limit) {
		Map map = threadLocal.get();

		ObjectPoolAdvanced<T> pool = (ObjectPoolAdvanced<T>) map.get(cls);
		if (pool == null) {
			pool = new ObjectPoolAdvanced(cls, limit);
			map.put(cls, pool);
		}
