...
"Input points appear to be coincident");
	   ...
	   double tCheck = 1d*tolerance;
		if (Math.sqrt(maxSqr) <= tCheck) {
			throw new IllegalArgumentException(
					"Input points appear to be coplanar: " + Math.sqrt(maxSqr) + " / " + tCheck);
		}
	   nrml.normalize();
...
//			System.err.println("DIST: "+dist+"; "+nrml+"; "+"; "+vtx[2].pnt+"; "+pointBuffer[i].pnt+"; dot: "+pointBuffer[i].pnt.dot(nrml)+"; d0 "+d0);
...
//		if (Math.abs(maxDist) <= tCheck) {
//			throw new IllegalArgumentException(
//					"Input points appear to be coplanar: " + Math.abs(maxDist) + " / " + tCheck);
//		}
