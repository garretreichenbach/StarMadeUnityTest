...
public class RemoteLongTransformationPairBuffer extends RemoteBuffer<RemoteLongTransformationPair> {

	private static final int CONTROL_BATCH = 32;

	public RemoteLongTransformationPairBuffer(NetworkObject synchOn) {
		super(RemoteLongTransformationPair.class, synchOn);
	}
	...
	public void fromByteStream(DataInputStream buffer, int updateSenderStateId) throws IOException {
		...
		for (int n = 0; n < collectionSize; n++) {
			RemoteLongTransformationPair instance = new RemoteLongTransformationPair(new LongTransformPair(), onServer);
	...
	public int toByteStream(DataOutputStream buffer) throws IOException {
		...
		for (int i = 0; i < batchSize; i++) {
			RemoteLongTransformationPair remoteField = get().remove(0);
			size++;
