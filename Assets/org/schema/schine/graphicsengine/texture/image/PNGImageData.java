package org.schema.schine.graphicsengine.texture.image;
...
public class PNGImageData implements LoadableImageData {
	...
	public long getSizeBef(InputStream fis) throws IOException {
		return getSizeBef(fis, false, null);
	}
	public long getSizeBef(InputStream fis, boolean flipped, int[] transparent) throws IOException {
		return getSizeBef(fis, flipped, false, transparent);
	}
	public long getSizeBef(InputStream fis, boolean flipped, boolean forceAlpha, int[] transparent) throws IOException{
		if (transparent != null) {
			forceAlpha = true;
		...
		return texWidth * texHeight * perPixel;
	}
	/**
	...
	public ByteBuffer loadImage(InputStream fis, boolean flipped, boolean forceAlpha, int[] transparent) throws IOException {
		...
		// Get a pointer to the image memory
		scratch = MemoryUtil.memAlloc(texWidth * texHeight * perPixel);
		...
		if (!decoder.hasAlpha() && forceAlpha) {
			ByteBuffer temp = MemoryUtil.memAlloc(texWidth * texHeight * 4);
			for (int x=0;x<texWidth;x++) {
				for (int y=0;y<texHeight;y++) {
