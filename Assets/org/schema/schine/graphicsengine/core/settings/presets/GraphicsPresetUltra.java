...
public class GraphicsPresetUltra extends EngineSettingsPreset{

	public GraphicsPresetUltra() {
		super("GRAPHICS_ULTRA");
	}

	@Override
	public String getName() {
		return Lng.str("Ultra");
	}
	...
	public void init() {
		...
		addSetting(EngineSettings.G_MULTI_SAMPLE, 16);
		...
		addSetting(EngineSettings.G_SHADOW_USE_PCF, true); // shadow quality
		...
		addSetting(EngineSettings.G_STAR_COUNT, 65536); // star count
		...
		addSetting(EngineSettings.LIGHT_RAY_COUNT, 128); // ray count for block baked light

		addSetting(EngineSettings.G_MAX_MISSILE_TRAILS, 2048); // max missile trails

		addSetting(EngineSettings.G_MAX_SEGMENTSDRAWN, 200000); // max segments

		addSetting(EngineSettings.G_MAX_BEAMS, 8192); // max segments
		...
		addSetting(EngineSettings.LOD_DISTANCE_IN_THRESHOLD, 500f); // lod distance
	}
}
