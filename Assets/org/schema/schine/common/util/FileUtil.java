...
//		System.err.println("DIR FROM " + sourceLocation.getAbsolutePath() + " to " + targetLocation.getAbsolutePath());
		if (sourceLocation.isDirectory()) {
			...
			for (int i = 0; i < children.length; i++) {
				...
				copyDirectory(new FileExt(sourceLocation, children[i]),
						new FileExt(targetLocation, children[i]));
			}
		} else {
	...
	public static boolean deleteDir(File dir) {
		if (dir.isDirectory()) {
			...
			for (int i = 0; i < children.length; i++) {
				boolean success = deleteDir(new FileExt(dir, children[i]));
				if (!success) {
					return false;
				}
			}
	...
	public static long getExtractedFilesSize(File zip) throws ZipException,
			IOException {
		long size = 0;
...
//				System.err.println("FILE SIZE: "+ze.getName()+": "+uncompressedSize);
	...
	public static void extract(File zip, String installPath, String replace, ZipCallback callback) throws IOException {
		...
		while (entries.hasMoreElements()) {
			...
			if (entry.isDirectory()) {
				...
				if (replace != null) {
					(new FileExt(installPath + entry.getName().replaceFirst(replace, ""))).mkdir();
				} else {
			...
			String s = new String(entry.getName());

			if (replace != null) {
				s = new String(entry.getName().replaceFirst(replace, ""));
			} else {
				s = new String(entry.getName());
			}
			...
			if (i >= 0) {
				...
				File p = new FileExt(installPath + path);
				if (!p.exists()) {
			...
			if (replace != null) {
				extr = new FileExt(installPath + entry.getName().replaceFirst(replace, ""));
			} else {
				extr = new FileExt(installPath + entry.getName());
			}
	...
	public static void main(String[] sdf) {
		try {
			extract(new FileExt("./sector-export/planet.smsec"), "./sector-export/mm/");
	...
	public static void copyURLToFile(URL source, File destination,
	                                 int connectionTimeout, int readTimeout, DownloadCallback cb) throws IOException {
	...
	public static void copyURLToFile(URL source, File destination,
	                                 int connectionTimeout, int readTimeout, DownloadCallback cb, String user, String pass, boolean resume) throws IOException {
...
//			connection.setRequestProperty ("Authorization", "Basic " + encoding);
	...
	public static Iterable<Class> getClasses(String packageName) throws ClassNotFoundException, IOException {
		...
		while (resources.hasMoreElements()) {
			...
			dirs.add(new FileExt(resource.getFile()));
		}
	 ...
	 * @since 1.3
	 */
	public static long copyLarge(InputStream input, OutputStream output, DownloadCallback cb)
			throws IOException {
	 ...
	 * @since 2.2
	 */
	public static long copyLarge(InputStream input, OutputStream output, byte[] buffer, DownloadCallback cb)
			throws IOException {
		long count = 0;
		int n = 0;
	...
	private static void createFilesHashRecursively(String path, FileFilter fn, ObjectArrayList<File> files) throws IOException {
		...
		if (f.exists() && f.isDirectory()) {
			...
			if (listFiles != null) {
				for (int i = 0; i < listFiles.length; i++) {
					if (listFiles[i].isDirectory()) {
						if (fn.accept(listFiles[i])) {
							createFilesHashRecursively(listFiles[i].getAbsolutePath(), fn, files);
						}
					} else {
						if (fn.accept(listFiles[i])) {
