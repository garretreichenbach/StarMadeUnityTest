...
import org.schema.game.network.objects.PlayerKeyConfig;
import org.schema.schine.network.objects.NetworkObject;
import org.schema.schine.network.objects.remote.RemoteBuffer;
import org.schema.schine.network.objects.remote.Streamable;

public class RemotePlayerKeyConfigBuffer extends RemoteBuffer<RemotePlayerKeyConfig> {

	private static final int CONTROL_BATCH = 16;

	public RemotePlayerKeyConfigBuffer(NetworkObject synchOn) {
		super(RemotePlayerKeyConfig.class, synchOn);
	}
	...
	public void fromByteStream(DataInputStream buffer, int updateSenderStateId) throws IOException {
		...
		for (int n = 0; n < collectionSize; n++) {
			RemotePlayerKeyConfig instance = new RemotePlayerKeyConfig(new PlayerKeyConfig(), onServer);
