...
public class AcidFormulaConeStartWide extends AcidDamageFormula{	
	...
	public void getAcidDamageSetting(short type, int damage, int restDamage, int initialDamage, float totalArmorValue, int blockIndex,
		...
		float acidDamageWeight = CannonElementManager.ACID_DAMAGE_FORMULA_CONE_START_WIDE_WEIGHT 
			+ 2 * (Math.max(0f, Math.min(1f, (float) blockIndex / (float) penetrationDepth))) * (1 - CannonElementManager.ACID_DAMAGE_FORMULA_CONE_START_WIDE_WEIGHT);
	...
	public AcidFormulaType getType() {
		return AcidFormulaType.CONE_START_WIDE;
	}
