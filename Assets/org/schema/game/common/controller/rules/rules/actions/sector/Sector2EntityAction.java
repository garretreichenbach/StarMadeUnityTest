...
public class Sector2EntityAction extends SectorAction{
	...
	public Sector2EntityAction() {
		super();
	}
	@Override
	public String getDescriptionShort() {
		return Lng.str("Execute %s entity actions for entities in sector", actions.size());
	}
	...
	public void onTrigger(RemoteSector s) {
		if(s.isOnServer()) {
			...
			for(SimpleTransformableSendableObject<?> p : s.getServerSector().getEntities()) {
				if(p.getSectorId() == s.getId() && p instanceof SegmentController) {
					actions.onTrigger((SegmentController)p);
				}
			}
		}
	}
	...
	public void onUntrigger(RemoteSector s) {
		if(s.isOnServer()) {
			...
			for(SimpleTransformableSendableObject<?> p : s.getServerSector().getEntities()) {
				if(p.getSectorId() == s.getId() && p instanceof SegmentController) {
					actions.onUntrigger((SegmentController)p);
				}
			}
		}
	}
