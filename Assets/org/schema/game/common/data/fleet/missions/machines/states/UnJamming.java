...
public class UnJamming extends FleetState{

	public UnJamming(Fleet gObj) {
		super(gObj);
	}
			...
			if(fleetMember.isLoaded() ){
				...
				if(!s.isCoreOverheating()){
					...
					if(!collection.getElementCollections().isEmpty()){
						//unjam
						s.getManagerContainer().getStealthElementManager().stopStealth(StealthElementManager.REUSE_DELAY_ON_SWITCHED_OFF_MS);
					}
				}
			}
		}
	...
	public FleetStateType getType() {
		return FleetStateType.UNJAMMING;
	}
