...
public class ElementCategory {
	...
	public String find(ElementInformation info) {
		if (this.equals(info.getType())) {
			...
			for (ElementCategory c : children) {
	...
	private void printItems(int lvl) {
		...
		for (int i = 0; i < infoElements.size(); i++) {
			ElementInformation elementInformation = infoElements.get(i);
	...
	public ElementCategory getChild(String nodeName) {
		for (int i = 0; i < children.size(); i++) {
			if (children.get(i).category.equals(nodeName)) {
	...
	public void addContextMenu(final ElementTreeInterface frame, JPopupMenu popup, Component parentComponent) {
		...
		{
			JMenuItem jBBMenuItem = new JMenuItem("Sort By Reactor Chamber Root");
			jBBMenuItem.addActionListener(e -> {
				Comparator<ElementInformation> m = (o1, o2) -> {
					int i = o1.chamberRoot - o2.chamberRoot;
					if(i == 0) {
				...
				Collections.sort(infoElements, m);
				frame.reinitializeElements();
			});
		...
		for (int i = 0; i < fields.length; i++) {
			...
			Element annotation = f.getAnnotation(org.schema.game.common.data.element.annotation.Element.class);
	...
	private void bulkCreateSlabs(boolean reinitializeOnly) {
		...
		for(ElementInformation info : elements){
			if(info.getSlab() == 0 && info.getBlockStyle() == BlockStyle.NORMAL && (!reinitializeOnly || info.slabIds != null)){
