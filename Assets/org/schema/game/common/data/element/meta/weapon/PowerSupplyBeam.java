...
public class PowerSupplyBeam extends Weapon {

	float speed = 70f;
	private int supplyPower = 50;
	...
	public PowerSupplyBeam(int id) {
		super(id, WeaponSubType.POWER_SUPPLY.type);
	}
	@Override
	...
	public void fromTag(Tag tag) {
		...
		supplyPower = Math.min(50, (Integer) v[0].getValue());
	...
	public Tag getBytesTag() {
		return new Tag(Type.STRUCT, null, new Tag[]{
				new Tag(Type.INT, null, supplyPower),
	...
	public void fire(AbstractCharacter<?> playerCharacter, AbstractOwnerState state, Vector3f dir, boolean addButton, boolean removeButton, Timer timer) {
		...
		if (playerCharacter instanceof PlayerCharacter) {
			((PlayerCharacter) playerCharacter).shootPowerSupplyBeam(((PlayerState) playerCharacter.getOwnerState()).getControllerState().getUnits().iterator().next(), supplyPower, this, addButton, removeButton);
		}
	}

	@Override
	protected String toDetailedString() {
		return Lng.str("Power Supply Beam\nPower: %s\nSpeed: %s\nColor(RGBA): %s",  supplyPower,  speed,  color);
	}
	...
	public boolean equalsObject(MetaObject other) {
		return super.equalsTypeAndSubId(other) && supplyPower == ((PowerSupplyBeam)other).supplyPower && color.equals(((PowerSupplyBeam)other).color) && speed == (((PowerSupplyBeam)other).speed);
	}
	@Override
