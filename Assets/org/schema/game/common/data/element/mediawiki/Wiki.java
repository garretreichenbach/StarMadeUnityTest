...
//        StringBuilder basegen = new StringBuilder("https://");
	...
	public synchronized void login(String username, char[] password) throws IOException, FailedLoginException {
		...
		System.err.println("#######TOKEN: "+wpLoginToken);
	...
	public Revision[] rangeContribs(String range) throws IOException {
		...
		if (numbers.length != 4)
			throw new NumberFormatException("Not a valid CIDR range!");
		return switch(size) {
			case 8 -> contribs("", numbers[0] + ".", null, null);
			case 16 -> contribs("", numbers[0] + "." + numbers[1] + ".", null, null);
			case 24 -> contribs("", numbers[0] + "." + numbers[1] + "." + numbers[2] + ".", null, null);
			case 32 -> contribs(range.substring(0, range.length() - 3), "", null, null);
			default -> throw new NumberFormatException("Range is not supported.");
		};
	}
	...
	protected String post(String url, String text, String caller) throws IOException {
		logurl(url, caller);
		System.err.println("URL: "+url+"; CAÖÖ:: "+text+"; CALL: "+caller);
		URLConnection connection = new URL(url).openConnection();
		setCookies(connection);
	...
	private String parseAttribute(String xml, String attribute, int index) {
		...
		System.err.println("LOGIN RET: "+xml);
	...
	public String normalize(String s) throws IOException {
		...
		for (int i = 0; i < temp.length; i++) {
			switch(temp[i]) {
				// illegal characters
				case '{', '}', '<', '>', '[', ']', '|' -> throw new IllegalArgumentException(s + " is an illegal title");
				case '_' -> temp[i] = ' ';
			}
		}
