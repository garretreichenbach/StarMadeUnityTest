...
import org.schema.game.common.data.world.planet.old.texgen.ColorForRunnable;
import org.schema.game.common.data.world.planet.old.texgen.MathUtil;
import org.schema.game.common.data.world.planet.old.texgen.palette.TerrainPalette;
...
public abstract class SpaceGenerator {
	...
	private SpacePalette m_palette;
	private int m_width, m_height;

	public SpaceGenerator(int texture_width, int texture_height, PlanetInformations info, SpacePalette color_palette) {
			...
			if (MultiThreadUtil.PROCESSORS_COUNT > 1) {
				...
				for (int x = 0; x < m_width; ++x) {
					for (int y = 0; y < m_height; ++y) {
						...
						int xy = Math.max(m_informations.getWaterLevel(), heightMap[at(x, y)]);
						int xyp1 = Math.max(m_informations.getWaterLevel(), heightMap[at(x, y + 1)]);
						int xp1y = Math.max(m_informations.getWaterLevel(), heightMap[at(x + 1, y)]);
	...
	protected float getTemperature(int y, int height) {
		...
		return 0;//257 + latitude * (m_informations.getEquatorTemperature() - m_informations.getPoleTemperature())
