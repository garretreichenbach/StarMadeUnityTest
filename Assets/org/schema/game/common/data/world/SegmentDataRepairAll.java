...
public class SegmentDataRepairAll implements SegmentDataInterface {
	...
	public static final int SEG_HALF = SEG / 2;
	...
	public SegmentDataRepairAll(boolean onClient) {
	...
	public static void setOrientation(int index, byte value, byte[] data) {
		assert (value >= 0 && value < 16) : "NOT A SIDE INDEX";
	 ...
	 */
	public int applySegmentData(Vector3b pos, byte[] pieceData, int offset, boolean synched, long absIndex, boolean updateSegmentBB, long time) {
		return applySegmentData(pos.x, pos.y, pos.z, pieceData, offset, synched, absIndex, updateSegmentBB, time);
	}
	 ...
	 * @return true if an element was added or removed
	 */
	public int applySegmentData(byte x, byte y, byte z, byte[] pieceData, int offset, final boolean synched, long absIndex, boolean updateSegmentBB, long time) {
		...
		try {
			...
			if (newType != oldType) {
				...
				if (oldType != Element.TYPE_NONE && newType == Element.TYPE_NONE) {
					onRemovingElement(index, x, y, z, oldType, updateSegmentBB, oldOrientation, oldActive, synched, time);
...
//		segment.setSegmentData(this);
	...
	public boolean equals(Object obj) {
		return segment.pos.equals(((SegmentDataRepairAll) obj).segment.pos);
	}
	...
	public void migrateTo(int fromVersion, SegmentDataInterface segmentData) {
		try{
			...
			for (int i = 0; i < BLOCK_COUNT; i++) {
				...
				segmentData.setType(index, getType(index));
				segmentData.setHitpointsByte(index, getHitpointsByte(index));
				segmentData.setActive(index, isActive(index));
				segmentData.setOrientation(index, getOrientation(index));
			...
			for (int i = 0; i < BLOCK_COUNT; i++) {
				...
				if (type != Element.TYPE_NONE && m.contains(type)) {
					segmentData.setHitpointsByte(index, ElementKeyMap.MAX_HITPOINTS);
				}
	...
	private void onAddingForcedElementUnsynched(int index, byte x, byte y, byte z, short newType, boolean updateSegmentBB, boolean segBufferUpdate) {

		this.blockAddedForced = true;
		//		// new element
		//		setSize(getSize() + 1);
		//
		//		if(newType == ElementKeyMap.DOOR_ELEMENT && !isActive(index)){
		//
		//		}else{
		//			getOctree().insert(x, y, z, this);
		//		}
		//
	...
	private void onAddingElementUnsynched(int index, byte x, byte y, byte z, short newType, boolean updateSegmentBB, boolean segBufferUpdate, long absIndex, long time) {
		...
		if (!(ElementKeyMap.infoArray[newType].isDoor() && !((data[index] & ACTIVE_BIT) == 0))) {
			octree.insert(x, y, z, index);
		}
	...
	public void onRemovingElement(int index, byte x, byte y, byte z, short oldType, boolean updateSegmentBB, byte oldOrientation, boolean wasActive, final boolean synched, long time) {
		...
		try {
			...
			// remove element
			setSize(oldSize - 1);
//			if(!ElementInformation.isPhysical(oldType, this, index)){
//			}else{
//				getOctree().delete(x, y, z, index, oldType);
//			}
	...
	public void restructBB(boolean updateSegmentBB) {
		...
		for (byte zC = 0; zC < SEG; zC++) {
			for (byte yC = 0; yC < SEG; yC++) {
				for (byte xC = 0; xC < SEG; xC++) {
...
//				getSegmentController().getSegmentBuffer().restructBBFast(this);
	...
	public void apply(RemoteSegment dequeue, boolean synched, SendableSegmentController c, long time) {
		if (synched) {
			rwl.writeLock().lock();
		}
		try {
			long xAbsStart = (dequeue.pos.x & 0xFFFF);
			long yAbsStart = ((long) (dequeue.pos.y & 0xFFFF) << 16);
			long zAbsStart = ((long) (dequeue.pos.z & 0xFFFF) << 32);
			long xAbs = xAbsStart;
			long yAbs = yAbsStart;
			long zAbs = zAbsStart;
			...
			for (byte z = 0; z < SEG; z++) {
				for (byte y = 0; y < SEG; y++) {
					for (byte x = 0; x < SEG; x++) {
						...
						short type = getType(dataIndex);
						short hitpoints = getHitpointsByte(dataIndex);
						//tested in elementCollection to be correct
						long absIndex = xAbs + yAbs + zAbs;
//						applySegmentData(x, y, z, dequeue.getSegmentData().data, dataIndex, false, absIndex, false, time);

						if (!c.isOnServer() && ((x % 2 == 0 && y % 2 == 0 && z % 2 == 0) || Math.random() > 0.9) && type > 0 && getType(dataIndex) == 0 && hitpoints > 0 && getHitpointsByte(dataIndex) == 0) {
							Vector3f f = new Vector3f();

							f.set(ElementCollection.getPosX(absIndex), ElementCollection.getPosY(absIndex), ElementCollection.getPosZ(absIndex));
							f.x -= SEG_HALF;
							f.y -= SEG_HALF;
							f.z -= SEG_HALF;

							//							((GameClientState)c.getState()).getWorldDrawer().getShards().voronoiBBShatter((PhysicsExt)((GameClientState)c.getState()).getPhysics(), tt, type, c.getSectorId(), tt.origin);

							((GameClientState) c.getState()).getWorldDrawer().getShards().voronoiBBShatterDelayed((PhysicsExt) ((GameClientState) c.getState()).getPhysics(), f, type, c, c.getGravity().source);
						}
...
//						short type = getType(index);
						...
						if (type != Element.TYPE_NONE) {
							...
							if (type <= ElementKeyMap.highestType && ElementKeyMap.validArray[type]) {
								...
								int maxHp = ElementKeyMap.MAX_HITPOINTS;

								if (getHitpointsByte(index) > maxHp) {
									setHitpointsByte(index, Math.min(maxHp, hp));
								}
//								onAddingElementUnsynched(index, x, y, z, (short)type, false, true, xAbs+yAbs+zAbs, time);
								...
								getSegmentController().onAddedElementSynched((short) type, getOrientation(index), x, y, z, this.segment, true, xAbs + yAbs + zAbs, time, false);
			...
			if (getSegmentController() instanceof EditableSendableSegmentController) {
				//				System.err.println("DOING EXT: "+min+"; "+max);
				((EditableSendableSegmentController) getSegmentController()).doDimExtensionIfNecessary(segment,
						min.x, min.y, min.z);
				((EditableSendableSegmentController) getSegmentController()).doDimExtensionIfNecessary(segment,
	...
	public void setInfoElementForcedAddUnsynched(byte x, byte y, byte z, short newType, byte orientation, byte activation, boolean updateSegmentBB) {
		...
		onAddingForcedElementUnsynched(index, x, y, z, newType, updateSegmentBB, false);
