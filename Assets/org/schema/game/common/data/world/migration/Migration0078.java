...
public class Migration0078 {
		...
		for (byte x = 0; x < 16; x++) {
			for (byte y = 0; y < 16; y++) {
				for (byte z = 0; z < 16; z++) {
					...
					int fromIndex = SegmentDataV0078.getInfoIndex(x, y, z);
	...
	public static void migrate0078to0079(File dir) throws IOException {
			...
			while (offsetIndex < headerTotalSize) {
				...
				try {
					if (offset >= 0) {
						...
						SegmentDataV0078 read = read0078(rf, offset, offsetIndex / headerArray.length, length, buffer);
	...
	public static SegmentDataV0078 read0078(DataInput dataInputStream) throws IOException {
		DataInputStream zip;
		if (size < 0) {
		...
		SegmentDataV0078 v = new SegmentDataV0078(false);
		v.deserialize(zip);
		return v;
	}

	private static SegmentDataV0078 read0078(RandomAccessFile rf,
		...
		SegmentDataV0078 s = read0078(in);
