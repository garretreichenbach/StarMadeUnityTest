...
import static com.bulletphysics.collision.broadphase.BroadphaseNativeType.FAST_CONCAVE_MESH_PROXYTYPE;
...
import static com.bulletphysics.collision.broadphase.BroadphaseNativeType.TERRAIN_SHAPE_PROXYTYPE;

import org.schema.game.common.data.physics.CubeConvexCollisionAlgorithm.CreateFunc;
...
public class CubeCollissionConfiguration extends CollisionConfiguration {
	...
	protected GjkEpaPenetrationDepthSolverExt pdSolver;
	...
	private CreateFunc cubesConvexCF;
	private CreateFunc convexCubesCF;
	...
	public CubeCollissionConfiguration() {
		...
		//#define USE_EPA 1
		//#ifdef USE_EPA
		pdSolver = new GjkEpaPenetrationDepthSolverExt();
		//#else
		...
		//default CreationFunctions, filling the m_doubleDispatch table
		 */
		convexConvexCreateFunc = new ConvexConvexExtAlgorithm.CreateFunc(simplexSolver, pdSolver);
		...
		compoundCreateFunc = new CompoundCollisionAlgorithmExt.CreateFunc();
		swappedCompoundCreateFunc = new CompoundCollisionAlgorithmExt.SwappedCreateFunc();
		...
		cubesCubesCF = new CubeCubeCollisionAlgorithm.CreateFunc(simplexSolver, pdSolver);
		cubesConvexCF = new CubeConvexCollisionAlgorithm.CreateFunc(simplexSolver, pdSolver);
		convexCubesCF = new CubeConvexCollisionAlgorithm.CreateFunc(simplexSolver, pdSolver);
		convexCubesCF.swapped = true;
	...
	public CollisionAlgorithmCreateFunc getCollisionAlgorithmCreateFunc(BroadphaseNativeType proxyType0, BroadphaseNativeType proxyType1) {
		...
		//		System.err.println("GETTING ALGO: "+proxyType0.name()+"; "+proxyType1.name());
		if ((proxyType0 == SPHERE_SHAPE_PROXYTYPE) && (proxyType1 == SPHERE_SHAPE_PROXYTYPE)) {
			return sphereSphereCF;
		}
		if ((proxyType0 == TERRAIN_SHAPE_PROXYTYPE) && (proxyType1 == TERRAIN_SHAPE_PROXYTYPE)) {
			return cubesCubesCF;
		}
		if ((proxyType0 == TERRAIN_SHAPE_PROXYTYPE) && proxyType1.isConvex()) {
			return cubesConvexCF;
		}
		if (proxyType0.isConvex() && (proxyType1 == TERRAIN_SHAPE_PROXYTYPE)) {
			return convexCubesCF;
		}
		/*
		...
		if (proxyType0 == FAST_CONCAVE_MESH_PROXYTYPE && proxyType1 == FAST_CONCAVE_MESH_PROXYTYPE) {
