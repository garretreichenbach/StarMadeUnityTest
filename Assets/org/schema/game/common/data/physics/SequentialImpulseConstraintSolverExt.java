...
public class SequentialImpulseConstraintSolverExt extends ConstraintSolver {
	...
	private static final int MAX_CONTACT_SOLVER_TYPES = 4;// HackeDiHack: ContactConstraintEnum.MAX_CONTACT_SOLVER_TYPES.ordinal();

	private static final int SEQUENTIAL_IMPULSE_MAX_SOLVER_POINTS = 128;//16384;
	...
	////////////////////////////////////////////////////////////////////////////
	private final ObjectPool<SolverBodyExt> bodiesPool = ObjectPool.get(SolverBodyExt.class);
	...
	private final ObjectArrayList<SolverBodyExt> tmpSolverBodyExtPool = new ObjectArrayList<SolverBodyExt>();
	...
	private int[] gOrder = new int[SEQUENTIAL_IMPULSE_MAX_SOLVER_POINTS * 2];
	public SequentialImpulseConstraintSolverExt() {
		this.v = new SequentialImpulseContraintSolverExtVariableSet();
	...
	protected void addFrictionConstraint(Vector3f normalAxis, int solverBodyIdA, int solverBodyIdB, int frictionIndex, ManifoldPoint cp, Vector3f rel_pos1, Vector3f rel_pos2, CollisionObject colObj0, CollisionObject colObj1, float relaxation) {
		...
		Vector3f ftorqueAxis1 = v.ftorqueAxis1;
		Matrix3f tmpMat = v.tmpMat;

		{
			...
			if (body0 != null) {
				...
				//				System.err.println("1TORQUE "+body0+": "+ftorqueAxis1);
		...
		}
		{
			...
			if (body1 != null) {
				...
				//				System.err.println("2TORQUE "+body1+": "+ftorqueAxis1);
	...
	private void initSolverBodyExt(SolverBodyExt solverBody, CollisionObject collisionObject) {
		RigidBody rb = RigidBody.upcast(collisionObject);
		if (rb != null) {
			rb.getAngularVelocity(solverBody.angularVelocity);
			solverBody.centerOfMassPosition.set(collisionObject.getWorldTransform(v.centerOfMassTrans).origin);
	...
	protected void prepareConstraints(PersistentManifold manifoldPtr, ContactSolverInfo info, IDebugDraw debugDrawer) {
		...
		// only necessary to refresh the manifold once (first iteration). The integration is done outside the loop
		{
			...
			Vector3f tmpVec = v.tmpVecB;
			Matrix3f tmpMat3 = v.tmpMat3B;

			Vector3f pos1 = v.pos1B;
			Vector3f pos2 = v.pos2B;
			Vector3f rel_pos1 = v.rel_pos1B;
			Vector3f rel_pos2 = v.rel_pos2B;
			Vector3f vel1 = v.vel1B;
			Vector3f vel2 = v.vel2B;
			Vector3f vel = v.velB;
			Vector3f totalImpulse = v.totalImpulseB;
			Vector3f torqueAxis0 = v.torqueAxis0B;
			Vector3f torqueAxis1 = v.torqueAxis1B;
			Vector3f ftorqueAxis0 = v.ftorqueAxis0B;
			Vector3f ftorqueAxis1 = v.ftorqueAxis1B;
			...
			for (int i = 0; i < numpoints; i++) {
				...
				if (cp.getDistance() <= 0f) {
					...
					Matrix3f mat1 = body0.getCenterOfMassTransform(v.com0).basis;
					mat1.transpose();

					Matrix3f mat2 = body1.getCenterOfMassTransform(v.com1).basis;
					mat2.transpose();

					JacobianEntry jac = jacobiansPool.get();
					jac.init(mat1, mat2,
							rel_pos1, rel_pos2, cp.normalWorldOnB,
							body0.getInvInertiaDiagLocal(v.in0), body0.getInvMass(),
							body1.getInvInertiaDiagLocal(v.in1), body1.getInvMass());
	...
	private float resolveSingleCollisionCombinedCacheFriendly(
...

		{
			...
			//			if(body1.originalBody.toString().contains("|CLI") && debug()){
			//			System.err.println("SOLVING "+body1.originalBody+" + "+body2.originalBody+"; \n" +
			//					"NORM "+contactConstraint.contactNormal+"" +
			//					"RES "+contactConstraint.restitution+"\n" +
			//					"NORM "+contactConstraint.originalContactPoint+"\n" +
			//					"NORM "+contactConstraint.appliedPushImpulse+"\n" +
			//							"");
			//			}
			//			System.err.println("CONTACT: "+contactConstraint.contactNormal+"; "+contactConstraint.penetration+"; "+body1.linearVelocity+"; "+body2.linearVelocity+"; POINT "+((ManifoldPoint)contactConstraint.originalContactPoint).positionWorldOnA+"; "+((ManifoldPoint)contactConstraint.originalContactPoint).localPointA);
			boolean ignore = false;
			if (body1.originalBody instanceof GamePhysicsObject) {
//				if(((GamePhysicsObject)body1.originalBody).getSimpleTransformableSendableObject().isOnServer()){
//					System.err.println("DOING A "+((GamePhysicsObject)body1.originalBody).getSimpleTransformableSendableObject().getState()+": "+body1.originalBody+" :: "+((GamePhysicsObject)body1.originalBody).getSimpleTransformableSendableObject());
//				}
				ignore = ((GamePhysicsObject) body1.originalBody).getSimpleTransformableSendableObject().handleCollision(0, body1.originalBody, body2.originalBody, contactConstraint);
			}
			if (body2.originalBody instanceof GamePhysicsObject) {
				GamePhysicsObject b = ((GamePhysicsObject)body2.originalBody);
				GameTransformable st = b.getSimpleTransformableSendableObject();
//				if(st.isOnServer()){
//					System.err.println("DOING B "+st.getState()+": "+b+" :: "+st);
//				}
				//always handle both
				ignore = ignore | st.handleCollision(1, body1.originalBody, body2.originalBody, contactConstraint);
			}
			//			if(ignore){
			//				System.err.println("IGNORE Collision Response");
			//				return 0.0f;
			//			}
			...
			boolean deb1 = body1.originalBody instanceof RigidDebrisBody;
			boolean deb2 = body2.originalBody instanceof RigidDebrisBody;
			
			boolean ai1 = body1.originalBody instanceof RigidBodySegmentController && ((RigidBodySegmentController)body1.originalBody).getSimpleTransformableSendableObject().isNPCFactionControlledAI();
			boolean ai2 = body2.originalBody instanceof RigidBodySegmentController && ((RigidBodySegmentController)body2.originalBody).getSimpleTransformableSendableObject().isNPCFactionControlledAI();
			
			boolean applyTo1 = (deb1 || !deb2) && (ai1 || !ai2); //deb1 -> deb2
			boolean applyTo2 = (deb2 || !deb1) && (ai2 || !ai1); //deb2 -> deb1
	...
	private float resolveSingleFrictionCacheFriendly(
		...
		if (appliedNormalImpulse > 0f) //friction
		{
			...
			Vector3f tmp = v.tmp3;
			if (body1.originalBody instanceof RigidDebrisBody || !(body2.originalBody instanceof RigidDebrisBody)) {
				tmp.scale(body1.invMass, contactConstraint.contactNormal);
				body1.internalApplyImpulse(tmp, contactConstraint.angularComponentA, j1);
			}

			if (body2.originalBody instanceof RigidDebrisBody || !(body1.originalBody instanceof RigidDebrisBody)) {
				tmp.scale(body2.invMass, contactConstraint.contactNormal);
	...
	public float resolveSingleCollision(
			RigidBody body1,
			RigidBody body2,
			ManifoldPoint contactPoint,
			ContactSolverInfo solverInfo) {

		Vector3f tmpVec = v.r_tmpVec;//new @Stack Vector3f();

		Vector3f pos1_ = contactPoint.getPositionWorldOnA(v.r_tt1);
		Vector3f pos2_ = contactPoint.getPositionWorldOnB(v.r_tt2);
		Vector3f normal = contactPoint.normalWorldOnB;

		// constant over all iterations
		Vector3f rel_pos1 = v.r_relPos1; //new @Stack Vector3f();
		rel_pos1.sub(pos1_, body1.getCenterOfMassPosition(tmpVec));

		Vector3f rel_pos2 = v.r_relPos2; //new @Stack Vector3f();
		rel_pos2.sub(pos2_, body2.getCenterOfMassPosition(tmpVec));

		Vector3f vel1 = body1.getVelocityInLocalPoint(rel_pos1, v.r_tt3);
		Vector3f vel2 = body2.getVelocityInLocalPoint(rel_pos2, v.r_tt4);
		Vector3f vel = v.r_vel;//new @Stack Vector3f();
		vel.sub(vel1, vel2);
		...
		Vector3f tmp = v.r_tmp;//new @Stack Vector3f();
		if (body1.getInvMass() != 0f) {
			...
			System.err.println("DOOOING A " + normalImpulse);
		}
		if (body2.getInvMass() != 0f) {
			...
			System.err.println("DOOOING B " + normalImpulse);
		}
			...
			if (cp.getDistance() <= 0f) {
				{
					...
					//schema edit:
					float impulse = resolveSingleCollision(body0, body1, cp, info);
			...
			int totalPoints = 0;
			{
				...
				for (j = 0; j < numManifolds; j++) {
					...
					for (short p = 0; p < manifoldPtr.getQuick(manifold_offset + j).getNumContacts(); p++) {
						gOrder[totalPoints * 2] = j;
						gOrder[totalPoints * 2 + 1] = p;
						//						gOrder[totalPoints].manifoldIndex = j;
						//						gOrder[totalPoints].pointIndex = p;
						totalPoints += 2;
						if (totalPoints >= gOrder.length) {
							int oldSize = gOrder.length;
							gOrder = Arrays.copyOf(gOrder, oldSize * 2);
						}
					}
				}
			}
			...
			int iteration;
			{
				for (iteration = 0; iteration < numiter; iteration++) {
					...
					if ((infoGlobal.solverMode & SolverMode.SOLVER_RANDMIZE_ORDER) != 0) {
						if ((iteration & 7) == 0) {
							for (j = 0; j < totalPoints; j += 2) {
								// JAVA NOTE: swaps references instead of copying values (but that's fine in this context)
								int manifold = gOrder[j];
								int point = gOrder[j + 1];
								int swapi = randInt2((j / 2) + 1);
								gOrder[j] = gOrder[swapi];
								gOrder[j + 1] = gOrder[swapi + 1];

								gOrder[swapi] = manifold;
								gOrder[swapi + 1] = point;
							}
						}
					}
					...
					for (j = 0; j < totalPoints; j += 2) {
						PersistentManifold manifold = manifoldPtr.getQuick(manifold_offset + gOrder[j]);
						solve((RigidBody) manifold.getBody0(),
								(RigidBody) manifold.getBody1(), manifold.getContactPoint(gOrder[j + 1]), info, iteration, debugDrawer);
					}
	...
	public float solveGroupCacheFriendly(ObjectArrayList<CollisionObject> bodies, int numBodies, ObjectArrayList<PersistentManifold> manifoldPtr, int manifold_offset, int numManifolds, ObjectArrayList<TypedConstraint> constraints, int constraints_offset, int numConstraints, ContactSolverInfo infoGlobal, IDebugDraw debugDrawer/*,btStackAlloc* stackAlloc*/) {
		//		if(debug()){
		//		for(int i = 0; i < manifoldPtr.size(); i++){
		//			PersistentManifold p = manifoldPtr.getQuick(i);
		//			if(p.getBody0().toString().contains("|CLI") || p.getBody1().toString().contains("|CLI")){
		//				System.err.println("RECEIVED MANIFOLDS "+p.getBody0()+"; "+p.getBody1()+": "+p.getNumContacts()+"; total manifolds: "+manifoldPtr.size()+"; "+p);
		//			}
		//		}
		//		}
		...
		for (int j = 0; j < numPoolConstraints; j++) {
			...
			//			System.err.println("DOING: "+pt.appliedImpulse+"; "+pt.appliedImpulseLateral1+"; "+pt.appliedImpulseLateral2);
			//			pt.appliedImpulse = 0;
			//			pt.appliedImpulseLateral1 = 0;
			//			pt.appliedImpulseLateral1 = 0;
			// do a callback here?
		}

		if (infoGlobal.splitImpulse) {
			for (int i = 0; i < tmpSolverBodyExtPool.size(); i++) {
				tmpSolverBodyExtPool.getQuick(i).writebackVelocity(infoGlobal.timeStep, v);
				bodiesPool.release(tmpSolverBodyExtPool.getQuick(i));
			}
		} else {
			for (int i = 0; i < tmpSolverBodyExtPool.size(); i++) {
				tmpSolverBodyExtPool.getQuick(i).writebackVelocity();
				bodiesPool.release(tmpSolverBodyExtPool.getQuick(i));
			}
		}
				...
				for (iteration = 0; iteration < infoGlobal.numIterations; iteration++) {
					...
					for (j = 0; j < numConstraints; j++) {
						...
						if ((constraint.getRigidBodyA().getIslandTag() >= 0) && (constraint.getRigidBodyA().getCompanionId() >= 0)) {
							tmpSolverBodyExtPool.getQuick(constraint.getRigidBodyA().getCompanionId()).writebackVelocity();
						}
						if ((constraint.getRigidBodyB().getIslandTag() >= 0) && (constraint.getRigidBodyB().getCompanionId() >= 0)) {
							tmpSolverBodyExtPool.getQuick(constraint.getRigidBodyB().getCompanionId()).writebackVelocity();
						}

						constraint.solveConstraint(infoGlobal.timeStep);

						if ((constraint.getRigidBodyA().getIslandTag() >= 0) && (constraint.getRigidBodyA().getCompanionId() >= 0)) {
							tmpSolverBodyExtPool.getQuick(constraint.getRigidBodyA().getCompanionId()).readVelocity();
						}
						if ((constraint.getRigidBodyB().getIslandTag() >= 0) && (constraint.getRigidBodyB().getCompanionId() >= 0)) {
							tmpSolverBodyExtPool.getQuick(constraint.getRigidBodyB().getCompanionId()).readVelocity();
						}
					}

					{
						...
						for (j = 0; j < numPoolConstraints; j++) {
							...
							resolveSingleCollisionCombinedCacheFriendly(tmpSolverBodyExtPool.getQuick(solveManifold.solverBodyIdA),
									tmpSolverBodyExtPool.getQuick(solveManifold.solverBodyIdB), solveManifold, infoGlobal);
						}
					}

					{
						...
						for (j = 0; j < numFrictionPoolConstraints; j++) {
							...
							resolveSingleFrictionCacheFriendly(tmpSolverBodyExtPool.getQuick(solveManifold.solverBodyIdA),
									tmpSolverBodyExtPool.getQuick(solveManifold.solverBodyIdB), solveManifold, infoGlobal,
									totalImpulse);
						}
					}
					...
					for (iteration = 0; iteration < infoGlobal.numIterations; iteration++) {
						{
							...
							for (j = 0; j < numPoolConstraints; j++) {
								SolverConstraint solveManifold = tmpSolverConstraintPool.getQuick(orderTmpConstraintPool.get(j));
								//								System.err.println("RESOLVING CONST "+solveManifold.contactNormal);
								resolveSplitPenetrationImpulseCacheFriendly(tmpSolverBodyExtPool.getQuick(solveManifold.solverBodyIdA),
										tmpSolverBodyExtPool.getQuick(solveManifold.solverBodyIdB), solveManifold, infoGlobal);
							}
						}
					}
	...
	public float solveGroupCacheFriendlySetup(ObjectArrayList<CollisionObject> bodies, int numBodies, ObjectArrayList<PersistentManifold> manifoldPtr, int manifold_offset, int numManifolds, ObjectArrayList<TypedConstraint> constraints, int constraints_offset, int numConstraints, ContactSolverInfo infoGlobal, IDebugDraw debugDrawer/*,btStackAlloc* stackAlloc*/) {
		...
		try {
			...
			//int sizeofSB = sizeof(btSolverBodyExt);
			...
			//if (1)
			{
				...
				//m_tmpSolverBodyExtPool.reserve(minReservation);
				...
				int solverBodyId = m_tmpSolverBodyExtPool.size();
				btSolverBodyExt& solverBody = m_tmpSolverBodyExtPool.expand();
				initSolverBodyExt(&solverBody,rb);
...

				{
					...
					Vector3f rel_pos1 = v.rel_pos1A;
					Vector3f rel_pos2 = v.rel_pos2A;

					Vector3f pos1 = v.pos1A;
					Vector3f pos2 = v.pos2A;
					Vector3f vel = v.velA;
					;
					Vector3f torqueAxis0 = v.torqueAxis0A;
					Vector3f torqueAxis1 = v.torqueAxis1A;
					;
					Vector3f vel1 = v.vel1A;
					Vector3f vel2 = v.vel2A;
					Vector3f frictionDir1 = v.frictionDir1A;
					Vector3f frictionDir2 = v.frictionDir2A;
					Vector3f vec = v.vecA;

					Matrix3f tmpMat = v.tmpMatA;
						...
						if (manifold.getNumContacts() != 0) {
							...
							if (colObj0.getIslandTag() >= 0) {
								if (colObj0.getCompanionId() >= 0) {
									...
									solverBodyIdA = tmpSolverBodyExtPool.size();
									SolverBodyExt solverBody = bodiesPool.get();
									tmpSolverBodyExtPool.add(solverBody);
									initSolverBodyExt(solverBody, colObj0);
							...
							if (colObj1.getIslandTag() >= 0) {
								if (colObj1.getCompanionId() >= 0) {
									...
									solverBodyIdB = tmpSolverBodyExtPool.size();
									SolverBodyExt solverBody = bodiesPool.get();
									tmpSolverBodyExtPool.add(solverBody);
									initSolverBodyExt(solverBody, colObj1);
...

								{
...

									{
										...
										if (rb0 != null) {
											...
											//											System.err.println("AAAVEC: "+vec+"; "+denom0);
										}
										if (rb1 != null) {
											vec.cross(solverConstraint.angularComponentB, rel_pos2);
											denom1 = rb1.getInvMass() + cp.normalWorldOnB.dot(vec);
											//											System.err.println("BBBVEC: "+vec+"; "+denom1);
										}
										//#endif //COMPUTE_IMPULSE_DENOM
									...
									//									System.err.println("VELOS IN LOCAL POINT "+vel1+"; "+vel2);
									vel.sub(vel1, vel2);
									...
									if ((infoGlobal.solverMode & SolverMode.SOLVER_USE_WARMSTARTING) != 0) {
										...
										if (rb0 != null) {
											...
											tmpSolverBodyExtPool.getQuick(solverConstraint.solverBodyIdA).internalApplyImpulse(tmp, solverConstraint.angularComponentA, solverConstraint.appliedImpulse);
										}
										if (rb1 != null) {
											tmp.scale(rb1.getInvMass(), solverConstraint.contactNormal);
											tmpSolverBodyExtPool.getQuick(solverConstraint.solverBodyIdB).internalApplyImpulse(tmp, solverConstraint.angularComponentB, -solverConstraint.appliedImpulse);
										}
									} else {
										solverConstraint.appliedImpulse = 0f;
									}
...

									{
										...
										if ((infoGlobal.solverMode & SolverMode.SOLVER_USE_WARMSTARTING) != 0) {
											...
											if (rb0 != null) {
												...
												tmpSolverBodyExtPool.getQuick(solverConstraint.solverBodyIdA).internalApplyImpulse(tmp, frictionConstraint1.angularComponentA, frictionConstraint1.appliedImpulse);
											}
											if (rb1 != null) {
												tmp.scale(rb1.getInvMass(), frictionConstraint1.contactNormal);
												tmpSolverBodyExtPool.getQuick(solverConstraint.solverBodyIdB).internalApplyImpulse(tmp, frictionConstraint1.angularComponentB, -frictionConstraint1.appliedImpulse);
											}
										} else {
									...
									}
									{
										...
										if ((infoGlobal.solverMode & SolverMode.SOLVER_USE_WARMSTARTING) != 0) {
											...
											if (rb0 != null) {
												...
												tmpSolverBodyExtPool.getQuick(solverConstraint.solverBodyIdA).internalApplyImpulse(tmp, frictionConstraint2.angularComponentA, frictionConstraint2.appliedImpulse);
											}
											if (rb1 != null) {
												tmp.scale(rb1.getInvMass(), frictionConstraint2.contactNormal);
												tmpSolverBodyExtPool.getQuick(solverConstraint.solverBodyIdB).internalApplyImpulse(tmp, frictionConstraint2.angularComponentB, -frictionConstraint2.appliedImpulse);
											}
										} else {
