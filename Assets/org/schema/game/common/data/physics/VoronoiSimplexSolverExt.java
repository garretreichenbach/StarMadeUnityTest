...
public class VoronoiSimplexSolverExt extends SimplexSolverInterface {
	...
	private static ThreadLocal<VoronoiSimplexSolverVariables> threadLocal = new ThreadLocal<VoronoiSimplexSolverVariables>() {
		@Override
		protected VoronoiSimplexSolverVariables initialValue() {
	...
	//	
	public static int pointOutsideOfPlane(Vector3f p, Vector3f a, Vector3f b, Vector3f c, Vector3f d, VoronoiSimplexSolverVariables v) {
		Vector3f tmp = v.tmpTmpTmp;//new @Stack Vector3f();

		Vector3f normal = v.normal;//new @Stack Vector3f();
		normal.sub(b, a);
		tmp.sub(c, a);
	...
	public boolean closestPtPointTetrahedron(Vector3f p, Vector3f a, Vector3f b, Vector3f c, Vector3f d, SubSimplexClosestResult finalResult) {
		...
		tempResult.reset();
		try {
			Vector3f tmp = v.tmptmptmptmp;//new @Stack Vector3f();
			Vector3f q = v.q;//new @Stack Vector3f();
			...
			int pointOutsideABC = pointOutsideOfPlane(p, a, b, c, d, v);
			int pointOutsideACD = pointOutsideOfPlane(p, a, c, d, b, v);
			int pointOutsideADB = pointOutsideOfPlane(p, a, d, b, c, v);
			int pointOutsideBDC = pointOutsideOfPlane(p, b, d, c, a, v);
	...
	public boolean closestPtPointTriangle(Vector3f p, Vector3f a, Vector3f b, Vector3f c, SubSimplexClosestResult result) {
		...
		Vector3f ab = this.v.ab;//new @Stack Vector3f();
		ab.sub(b, a);

		Vector3f ac = this.v.ac;//new @Stack Vector3f();
		ac.sub(c, a);

		Vector3f ap = this.v.ap;//new @Stack Vector3f();
		ap.sub(p, a);
		...
		Vector3f bp = this.v.bp;//new @Stack Vector3f();
		bp.sub(p, b);
		...
		Vector3f cp = this.v.cp;//new @Stack Vector3f();
		cp.sub(p, c);
		...
		if (va <= 0f && (d4 - d3) >= 0f && (d5 - d6) >= 0f) {
			...
			Vector3f tmp = this.v.tmptmp;//new @Stack Vector3f();
			tmp.sub(c, b);
		...
		Vector3f tmp1 = this.v.tmptmp1;//new @Stack Vector3f();
		Vector3f tmp2 = this.v.tmptmp2;//new @Stack Vector3f();
	...
	public boolean updateClosestVectorAndPoints() {
		if (needsUpdate) {
			...
			needsUpdate = false;
			switch(numVertices()) {
				case 0 -> cachedValidClosest = false;
				case 1 -> {
				...
				case 2 -> {
					...
					Vector3f nearest = v.nearest;//new @Stack Vector3f();
					Vector3f p = v.p;//new @Stack Vector3f();
					p.set(0f, 0f, 0f);
					Vector3f diff = v.diff;//new @Stack Vector3f();
					diff.sub(p, from);
					Vector3f v = this.v.v;//new @Stack Vector3f();
					v.sub(to, from);
					float t = v.dot(diff);
					if(t > 0) {
						float dotVV = v.dot(v);
				...
				case 3 -> {
					Vector3f tmp1 = this.v.tmp1;//new @Stack Vector3f();
					Vector3f tmp2 = this.v.tmp2;//new @Stack Vector3f();
					Vector3f tmp3 = this.v.tmp3;// new @Stack Vector3f();
					// closest point origin from triangle
					Vector3f p = this.v.p;//new @Stack Vector3f();
					p.set(0f, 0f, 0f);
				...
				case 4 -> {
					...
					Vector3f tmp2 = this.v.tmp2;//new @Stack Vector3f();
					Vector3f tmp3 = this.v.tmp3;//new @Stack Vector3f();
					Vector3f tmp4 = this.v.tmp4;//new @Stack Vector3f();
					Vector3f p = this.v.p;//new @Stack Vector3f();
					p.set(0f, 0f, 0f);
					Vector3f a = simplexVectorW[0];
					Vector3f b = simplexVectorW[1];
