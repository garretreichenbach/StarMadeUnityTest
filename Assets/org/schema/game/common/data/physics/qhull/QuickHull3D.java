 ...
 * {@link #build(DPoint3d[]) build} method. After
...
public class QuickHull3D {
	 ...
	 * coplanar.
	 */
	public QuickHull3D(DPoint3d[] points)
			throws IllegalArgumentException {
	...
	private void addPointToFace(DVertex vtx, Face face) {
		vtx.face = face;
	...
	private void removePointFromFace(DVertex vtx, Face face) {
		if (vtx == face.outside) {
	...
	private DVertex removeAllPointsFromFace(Face face) {
		if (face.outside != null) {
			DVertex end = face.outside;
	...
	private HalfEdge findHalfEdge(DVertex tail, DVertex head) {
	...
	protected void setFromQhull(double[] coords, int nump,
		...
		try {
			...
			if (stok.ttype != StreamTokenizer.TT_NUMBER) {
				...
				try{throw new Exception("System.exit() called");}catch(Exception ex){ex.printStackTrace();}System.exit(1);
			}
	 ...
	 * coplanar.
	 */
	public void build(DPoint3d[] points)
			throws IllegalArgumentException {
		build(points, points.length);
	}
	 ...
	 */
	public void build(DPoint3d[] points, int nump)
			throws IllegalArgumentException {
		if (nump < 4) {
			throw new IllegalArgumentException(
					"Less than four input points specified");
	...
	protected void setPoints(double[] coords, int nump) {
		for (int i = 0; i < nump; i++) {
			DVertex vtx = pointBuffer[i];
			vtx.pnt.set(coords[i * 3 + 0], coords[i * 3 + 1], coords[i * 3 + 2]);
			vtx.index = i;
		}
	}

	protected void setPoints(DPoint3d[] pnts, int nump) {
		for (int i = 0; i < nump; i++) {
			DVertex vtx = pointBuffer[i];
			vtx.pnt.set(pnts[i]);
			vtx.index = i;
	...
	protected void computeMaxAndMin() {
		DVector3d max = new DVector3d();
		DVector3d min = new DVector3d();
		...
		for (int i = 1; i < numPoints; i++) {
			DPoint3d pnt = pointBuffer[i].pnt;
			if (pnt.x > max.x) {
				max.x = pnt.x;
	...
	protected void createInitialSimplex()
		...
		DVector3d u01 = new DVector3d();
		DVector3d diff02 = new DVector3d();
		DVector3d nrml = new DVector3d();
		DVector3d xprod = new DVector3d();
		double maxSqr = 0;
	...
	public DPoint3d[] getVertices() {
		DPoint3d[] vtxs = new DPoint3d[numVertices];
	...
	public int getVertices(double[] coords) {
		for (int i = 0; i < numVertices; i++) {
			DPoint3d pnt = pointBuffer[vertexPointIndices[i]].pnt;
			coords[i * 3 + 0] = pnt.x;
			coords[i * 3 + 1] = pnt.y;
			coords[i * 3 + 2] = pnt.z;
		}
	...
	protected void resolveUnclaimedPoints(FaceList newFaces) {
		DVertex vtxNext = unclaimed.first();
		for (DVertex vtx = vtxNext; vtx != null; vtx = vtxNext) {
			vtxNext = vtx.next;
	...
	protected void deleteFacePoints(Face face, Face absorbingFace) {
		DVertex faceVtxs = removeAllPointsFromFace(face);
		if (faceVtxs != null) {
			if (absorbingFace == null) {
				unclaimed.addAll(faceVtxs);
			} else {
				DVertex vtxNext = faceVtxs;
				for (DVertex vtx = vtxNext; vtx != null; vtx = vtxNext) {
					vtxNext = vtx.next;
	...
	protected void calculateHorizon(
			DPoint3d eyePnt, HalfEdge edge0, Face face, Vector horizon) {
		//	   oldFaces.add (face);
	...
	protected void addNewFaces(
			FaceList newFaces, DVertex eyeVtx, Vector horizon) {
		newFaces.clear();
	...
	protected DVertex nextPointToAdd() {
		if (!claimed.isEmpty()) {
			...
			DVertex eyeVtx = null;
			double maxDist = 0;
			for (DVertex vtx = eyeFace.outside;
	...
	protected void addPointToHull(DVertex eyeVtx) {
		horizon.clear();
		unclaimed.clear();
