...
public class QuickHull3DTest {
	...
	private void randomlyPerturb(DPoint3d pnt, double tol) {
	...
	void explicitFaceCheck(QuickHull3D hull, int[][] checkFaces)
		...
		DPoint3d[] pnts = hull.getVertices();
	...
	void testException(double[] coords, String msg) {
		...
		if (ex == null) {
			...
			printCoords(coords);
			try{throw new Exception("System.exit() called");}catch(Exception ex1){ex1.printStackTrace();}System.exit(1);
			...
			System.out.println("Input pnts:");
			printCoords(coords);
			try{throw new Exception("System.exit() called");}catch(Exception ex2){ex2.printStackTrace();}System.exit(1);
		}
	}
