...
public class GjkEpaSolverExt {
	...
	public static class Results {
		...
		public void reset() {
			status = null;
			witnesses[0].set(0,0,0);
			witnesses[1].set(0,0,0);
			normal.set(0,0,0);
			depth = 0;
			epa_iterations = 0;
			gjk_iterations = 0;
	...
	////////////////////////////////////////////////////////////////////////////
	private final EPA epa = new EPA();

	private final GJK gjk = new GJK();

	private final Mkv swapTmp = new Mkv();
	////////////////////////////////////////////////////////////////////////////
	...
	protected class GJK {
		...
		public void Support(Vector3f d, Mkv v) {
			...
			Vector3f tmp1 = LocalSupport(d, 0, q.tmp1);

			Vector3f tmp = q.tmp2;
			tmp.set(d);
			tmp.negate();
			Vector3f tmp2 = LocalSupport(tmp, 1, q.tmp3);
		...
		public boolean SolveSimplex4(Vector3f ao, Vector3f ab, Vector3f ac, Vector3f ad) {
			...
			Vector3f tmp2 = q.tmp10;
			tmp2.cross(ac, ad);

			Vector3f tmp3 = q.tmp11;
			tmp3.cross(ad, ab);
		...
		public boolean SearchOrigin() {
			Vector3f tmp = q.tmp12;
			tmp.set(1f, 0f, 0f);
			return SearchOrigin(tmp);
		}
		
		public boolean SearchOrigin(Vector3f initray) {
			Vector3f tmp1 = q.tmp13;
			Vector3f tmp2 = q.tmp14;
			Vector3f tmp3 = q.tmp15;
			Vector3f tmp4 = q.tmp16;
			...
			for (; iterations < GJK_maxiterations; ++iterations) {
				float rl = FastMath.carmackLength(ray);
		...
		public boolean EncloseOrigin() {
			Vector3f tmp = q.tmp17;
			Vector3f tmp1 = q.tmp18;
			Vector3f tmp2 = q.tmp19;
			...
			switch (order) {
				...
				case 1: {
					...
					q.b[0].cross(ab, q.b[0]);
					q.b[1].cross(ab, q.b[1]);
					q.b[2].cross(ab, q.b[2]);


					float l0 = q.b[0].lengthSquared();
					float l1 = q.b[1].lengthSquared();
					float l2 = q.b[2].lengthSquared();
					
					Quat4f tmpQuat = q.tmp24;
					tmp.normalize(ab);
					...
					Vector3f w = q.tmp26;
					w.set(q.b[l0 > l1 ? l0 > l2 ? 0 : 2 : l1 > l2 ? 1 : 2]);
	...
	protected class EPA {
		...
		public void reset(GJK pgjk){
			...
			nearest[0].set(0,0,0);
			nearest[1].set(0,0,0);
			//features are confirmed to be overwritten
			gjk = pgjk;
		}
		...
		public Vector3f GetCoordinates(Face face, Vector3f out) {
			Vector3f tmp = q.tmp28;
			Vector3f tmp1 = q.tmp29;
			Vector3f tmp2 = q.tmp30;
			...
			tmp.cross(tmp1, tmp2);
			a[0] = FastMath.carmackLength(tmp);

			tmp1.sub(face.v[1].w, o);
			tmp2.sub(face.v[2].w, o);
			tmp.cross(tmp1, tmp2);
			a[1] = FastMath.carmackLength(tmp);

			tmp1.sub(face.v[2].w, o);
			tmp2.sub(face.v[0].w, o);
			tmp.cross(tmp1, tmp2);
			a[2] = FastMath.carmackLength(tmp);
		...
		public boolean Set(Face f, Mkv a, Mkv b, Mkv c) {
			Vector3f tmp1 = q.tmp32;
			Vector3f tmp2 = q.tmp33;
			Vector3f tmp3 = q.tmp34;

			Vector3f nrm = q.tmp35;
			tmp1.sub(b.w, a.w);
			tmp2.sub(c.w, a.w);
			nrm.cross(tmp1, tmp2);

			float len = FastMath.carmackLength(nrm);
		...
		private final Mkv[] basemkv = new Mkv[5];
		{
			for(int i = 0; i < basemkv.length; i++){
				basemkv[i] = new Mkv();
			}
		}
		Face[] cf = new Face[]{null};
		Face[] ff = new Face[]{null};
		private final Face[] basefaces = new Face[6];
		public float EvaluatePD(float accuracy) {
			pushStack();
			try {
				Vector3f tmp = q.tmp36;
				...
				/* Extract contact	*/
				if (bestface != null) {
					Vector3f b = GetCoordinates(bestface, q.tmp38);
					...
					Vector3f tmp1 = q.tmp39;
					Vector3f tmp2 = q.tmp40;
					Vector3f tmp3 = q.tmp41;
