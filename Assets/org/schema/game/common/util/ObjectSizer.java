...
public final class ObjectSizer {
	...
	public static long getObjectSize(Class<?> aClass) {
		...
		try {
			...
			result = FastMath.round(approximateSize);
		} catch (Exception ex) {
	...
	/**
	 * First and only argument is the package-qualified description of a class
	 * which has a no-argument constructor.
	 */
	public static void main(String... aArguments) {
		Class<?> theClass = null;
		try {
			theClass = Element.class;
		} catch (Exception ex) {
			System.err.println("Cannot build a Class object: " + aArguments[0]);
			System.err.println("Use a package-qualified description, and check classpath.");
		}
