...
@BlockShape(name = "SpikeBackFrontRight")
public class SpikeBackFrontRight extends SpikeShapeAlgorithm {

	private final int[] sidesToTest = new int[]{Element.FRONT};
	private org.schema.game.common.data.physics.ConvexHullShapeExt shape;

	public SpikeBackFrontRight() {
		super();
	...
	protected int[][] getNormals(){
		return new int[][]{{Element.BACK, Element.TOP}, {Element.BACK, Element.RIGHT}}; 
	}
	@Override
	...
	public void createSide(int sideId, short i, AlgorithmParameters p) {
		...
		switch(sideId) {
			case (Element.TOP) -> {
				p.normalMode = (Element.BACK + 1) * 64 + (Element.TOP + 1) * 8;
				if(p.vID == 0) {
