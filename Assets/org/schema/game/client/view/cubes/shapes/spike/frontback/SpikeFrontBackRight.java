...
@BlockShape(name = "SpikeFrontBackRight")
public class SpikeFrontBackRight extends SpikeShapeAlgorithm {
	...
	protected int[][] getNormals(){
		return new int[][]{{Element.FRONT, Element.BOTTOM}, {Element.FRONT, Element.LEFT}}; 
	}
	@Override
	...
	public void createSide(int sideId, short i, AlgorithmParameters p) {
		...
		switch(sideId) {
			...
			case (Element.BOTTOM) -> {
				p.normalMode = (Element.FRONT + 1) * 64 + (Element.BOTTOM + 1) * 8;
				if(p.vID == 0) {
					p.sid = Element.TOP;
					p.vID = (short) 3;
				}
