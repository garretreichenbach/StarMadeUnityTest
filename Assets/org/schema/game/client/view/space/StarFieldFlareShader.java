...
public class StarFieldFlareShader implements Shaderable {
	...
	public void updateShaderParameters(Shader shader) {
		if (starTexId < 0) {
			starTexId = Controller.getResLoader().getSprite("flare").getMaterial().getTexture().getTextureId();
		}
		if (starColorId < 0) {
			starColorId = Controller.getResLoader().getSprite("starColorMap-gui-").getMaterial().getTexture().getTextureId();
		}
		GlUtil.updateShaderVector4f(shader, "Param", new Vector4f(230f, 0.31f, 210f, 0.001f));
