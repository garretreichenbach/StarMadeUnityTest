...
public class GUIOrientationSettingElement extends GUISettingsElement {
	...
	public GUIOrientationSettingElement(InputState state) {
		super(state);
	...
	public void onInit() {
		...
		leftArrow.setCallback(new GUICallback() {
			...
			public void callback(GUIElement callingGuiElement, MouseEvent event) {
				if (event.pressedLeftMouse()) {
					...
					AudioController.fireAudioEventID(355);
		...
		rightArrow.setCallback(new GUICallback() {
			...
			public void callback(GUIElement callingGuiElement, MouseEvent event) {
				if (event.pressedLeftMouse()) {
					/*AudioController.fireAudioEvent(AudioTags.GUI, AudioTags.BUTTON, AudioTags.PRESS, AudioTags.SELECT)*/
					AudioController.fireAudioEventID(356);
					int blockOrientation = getPlayerInteractionControlManager().getBlockOrientation();
					short type = getPlayerInteractionControlManager().getSelectedTypeWithSub();
		...
		blockPreview.getPos().x = leftArrow.getWidth() + 29;
		blockPreview.getPos().y = 24;
		blockBg.setPos(blockPreview.getPos().x - 26, blockPreview.getPos().y - 26, 0);
		rightArrow.getPos().x = leftArrow.getWidth() + 60;
		init = true;
	}
	...
	public float getHeight() {
		return blockPreview.getHeight() + 12;
	}
