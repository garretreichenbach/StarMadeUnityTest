...
public class BuildingWorld {
	...
	static int[][] world = new int[WORLD_SIZE][WORLD_SIZE];
	...
	static int build_light_strip(int x1, int z1, int direction) {
		...
		if (direction == Building.EAST) {
			d.CreateLightStrip(x1, z1 - size_adjust, width, depth + size_adjust, 2, color);
		} else if (direction == Building.WEST) {
			d.CreateLightStrip(x1, z1, width, depth + size_adjust, 2, color);
		} else if (direction == Building.NORTH) {
			d.CreateLightStrip(x1, z1, width + size_adjust, depth, 2, color);
		} else {
			d.CreateLightStrip(x1 - size_adjust, z1, width + size_adjust, depth, 2, color);
		}
		return length;
	...
	static boolean claimed(int x, int y, int width, int depth) {
	...
	void do_building(Plot p) {
		...
		if (square && p.width > 20) {
			...
			modern_count++;
			skyscrapers++;
			buildings.add(new Building(Building.BUILDING_MODERN, p.x, p.z, height, p.width, p.depth, seed, color));
			return;
		}
		/*
	  ...
	  if (multiTexturePathPattern.width > multiTexturePathPattern.depth * 2 || multiTexturePathPattern.depth > multiTexturePathPattern.width * 2 && area > 800) {
		...
		height = 45 + random.nextInt(10);
		buildings.add(new Building(type, p.x, p.z, height, p.width, p.depth, seed, color));
		skyscrapers++;
	...
	public void do_reset() {
		...
		for (x = 1; x < WORLD_SIZE - 1; x++) {
			for (y = 0; y < WORLD_SIZE; y++) {
				...
				y += build_light_strip(x, y, road_right ? Building.SOUTH : Building.NORTH);
			}
		}
		...
		for (y = 1; y < WORLD_SIZE - 1; y++) {
			for (x = 1; x < WORLD_SIZE - 1; x++) {
				...
				x += build_light_strip(x, y, road_right ? Building.EAST : Building.WEST);
			}
		}
				...
				while (width > 8 && depth > 8) {
					if (!claimed(x, y, width, depth)) {
						...
						if (x < hot_zone.min.x || x > hot_zone.max.x || y < hot_zone.min.z || y > hot_zone.max.z) {
							...
							buildings.add(new Building(Building.BUILDING_SIMPLE, x + 1, y + 1, height, width - 2, depth - 2, random.nextInt(), building_color));
						} else { //use fancy buildings.
							height = 15 + random.nextInt(15);
							width -= 2;
							depth -= 2;
							if (coinflip()) {
								buildings.add(new Building(Building.BUILDING_TOWER, x + 1, y + 1, height, width, depth, random.nextInt(), building_color));
							} else {
								buildings.add(new Building(Building.BUILDING_BLOCKY, x + 1, y + 1, height, width, depth, random.nextInt(), building_color));
							}
						}
						break;
					}
