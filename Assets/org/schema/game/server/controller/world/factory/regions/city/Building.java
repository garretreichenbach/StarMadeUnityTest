...
public class Building {
	...
	public static final int ADDON_NONE = 0, ADDON_LOGO = 0, ADDON_TRIM = 0, ADDON_LIGHTS = 0, ADDON_COUNT = 5;
	...
	public Building(BuildingWorld world, Random random, Collection<Region> regions, int type, int x, int y, int height, int width, int depth, int seed, Vector4f color) {
		this.world = world;
		this.random = random;
	...
	public void ConstructCube(float left, float right, float front, float back, float bottom, float top) {
		...
		regions.add(new BuildingRegion(world, null, new Vector3i((int) left, (int) bottom, (int) back), new Vector3i((int) right, (int) top, (int) front), 8, 0));
	}
	...
	void ConstructRoof(float left, float right, float front, float back, float bottom) {
		...
		if(addon == ADDON_LOGO && !this.have_logo) {
			...
			CDeco d = new CDeco(world);
			regions.add(new DownSpikeRegion(null, new Vector3i((int) left, (int) bottom, (int) back), new Vector3i((int) right, (int) bottom + 10, (int) front), 8, 0));
			decos.add(d);
		...
		if(this.height > 45) {
			...
			d.CreateRadioTower(regions, new Vector3f((left + right) / 2.0f, bottom, (front + back) / 2.0f), 15.0f);
			decos.add(d);
		}
	}

	public void ConstructSimpleCube(float left, float right, float front, float back, float bottom, float top) {
		//	  GLvertex    multiTexturePathPattern[10];
		float x1, x2, z1, z2, y1, y2;
		int i;
		//	  cube        c;
		...
		z2 = back;
		//	  base_index = this.mesh_flat->VertexCount ();
		regions.add(new BuildingSimpleRegion(world, null, new Vector3i((int) left, (int) bottom, (int) back), new Vector3i((int) right, (int) top, (int) front), 8, 0, random));
	}
	...
	public void ConstructSpike(int left, int right, int front, int back, int bottom, int top) {
		...
		float centerx = (left + -(float) right) / 2.0f;
		float centerz = (front + -(float) back) / 2.0f;
		regions.add(new DownSpikeRegion(null, new Vector3i((int) centerx, bottom, (int) centerz), new Vector3i((int) centerx, top, (int) centerz), 8, 0));
	}
	...
	public float ConstructWall(int start_x, int start_y, int start_z, int direction, int length, int height, int window_groups, float uv_start, boolean blank_corners) {
		...
		uv.x = uv_start;
		blank = false;
		regions.add(new BuildingWallRegion(world, null, new Vector3i(start_x, start_y, start_z), new Vector3i(start_x + (step_x == 0 ? 1 : 0) + step_x * length, start_y + height, start_z + (step_z == 0 ? 1 : 0) + step_z * length), 8, 0));
	...
	void CreateModern() {
		...
		points = 0;
		skip_counter = 0;
		System.err.println("MODERN " + center + "; " + radius + "; " + y + "; " + height);
		regions.add(new BuildingModernRegion(world, null, new Vector3i((int) (center.x - radius.x), 0, (int) (center.z - radius.y)), new Vector3i((int) (center.x + radius.x), height, (int) (center.z - radius.y)), 8, 0));
		//TODO add modern building
	...
	void CreateSimple() {
		...
		regions.add(new BuildingSimpleRegion(world, null, new Vector3i((int) x1, (int) y1, (int) z1), new Vector3i((int) x2, (int) y2, (int) z2), 8, 0, random));
		u = (float) (random.nextInt(BuildingTexture.SEGMENTS_PER_TEXTURE / 2) + BuildingTexture.SEGMENTS_PER_TEXTURE / 2) / BuildingTexture.SEGMENTS_PER_TEXTURE;
		...
		ConstructSimpleCube(x1 - ledge, x2 + ledge, z2 - ledge, z1 + ledge, this.height, this.height + cap_height);
		//	  this.mesh->Compile ();
	}
