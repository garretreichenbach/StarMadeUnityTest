...
public class HangarRegion extends Region {

	public static float distUpper = 5;
	...
	public HangarRegion(Region[] regions, Vector3i min, Vector3i max,
	...
	private short placeDownPart() {
		...
		if (relPos.y == relMax.y / 2) {
			normal = ElementKeyMap.HULL_COLOR_BLACK_ID;
		} else if (relPos.y == relMax.y / 2 + 1 || relPos.y == relMax.y / 2 - 1) {
			normal = ElementKeyMap.HULL_COLOR_RED_ID;
		} else if (relPos.z % 8 == 0) {
			normal = ElementKeyMap.HULL_COLOR_GREEN_ID;
		}
		...
		if (relPos.x == relMax.x - 1 || relPos.x == 0 || relPos.z == relMax.z - 1) {
			return normal;
		}
